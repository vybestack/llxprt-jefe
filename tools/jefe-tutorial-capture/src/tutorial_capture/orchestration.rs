...
fn write_system_tool_links(shim_dir: &Path) -> Result<(), OrchestrationError> {
    ...
    for link in &links {
        ...
        // Remove stale link if it exists. NotFound is acceptable (first run);
        // other errors propagate so a permissions issue is surfaced.
        remove_stale_path(&link_path)?;
...
/// Remove a stale symlink/file, treating `NotFound` as success (the path
/// does not exist yet). Other I/O errors propagate so unexpected failures
/// (permissions, dangling parent) are not silently swallowed.
fn remove_stale_path(path: &Path) -> Result<(), OrchestrationError> {
...
pub fn cleanup_manifest(
    ...
    // Derive the run root as the common parent directory of all owned paths.
    // The sentinel file lives in the run root, so we need the directory that
    // *contains* the owned paths, not one of the owned paths themselves.
    let run_root = derive_run_root(&manifest.owned_paths);
    cleanup_manifest_with_root(manifest, &run_root, purge_evidence)
}

/// Compute the common parent directory of all owned paths.
///
/// Each owned path is a child of the run root (e.g. `<root>/config`,
/// `<root>/artifacts`). The run root is their shared parent. We find it by
/// taking the parent of the shortest path, which is guaranteed to be the
/// common ancestor because all owned paths are direct children of the root.
fn derive_run_root(owned_paths: &[crate::manifest::OwnedPath]) -> PathBuf {
    if owned_paths.is_empty() {
        return PathBuf::new();
    }
    // Find the shortest owned path and return its parent — this is the
    // run root because all owned paths are direct children of the root.
    let shortest = owned_paths
        .iter()
        .min_by_key(|p| p.path.components().count())
        .unwrap_or_else(|| panic!("owned_paths is non-empty (checked above)"));
    shortest
        .path
        .parent()
        .map(PathBuf::from)
...
/// Redact a single file atomically. Returns 1 if the file was modified, 0 if
/// unchanged.
///
/// Uses `atomic_write` (temp-file + rename) so a crash never leaves a
/// partially-written redacted file.
fn redact_single_file(path: &Path, set: &RedactionSet) -> Result<usize, OrchestrationError> {
    ...
    // Finding: write redacted content atomically (temp + rename) so a crash
    // during redaction cannot leave a partially-written file that could
    // expose unredacted content mixed with redacted content.
    persistence::atomic_write(path, &redacted).map_err(|e| match e {
        PersistenceError::Io { path: p, reason } => {
            OrchestrationError::Redaction(RedactionError::WriteFailed { path: p, reason })
        }
        other => OrchestrationError::Redaction(RedactionError::WriteFailed {
            path: path.to_string_lossy().into_owned(),
