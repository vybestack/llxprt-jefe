...
use super::*;
use crate::allowlist::FixtureAllowlist;
...
fn cleanup_rejects_arbitrary_branch_and_non_numeric_issue() {
    ...
    assert!(validate_resource_for_cleanup(&branch, &manifest, None).is_err());
    assert!(validate_resource_for_cleanup(&issue, &manifest, None).is_err());
}
...
fn cleanup_treats_already_deleted_branch_as_idempotent_success() {
    ...
    manifest.add_github_resource(GitHubResource {
        kind: GitHubResourceKind::Branch,
        repository: "fixture/test".to_string(),
        identifier: "tutorial-capture/cleanup-test".to_string(),
        url: None,
        title: String::new(),
    });
