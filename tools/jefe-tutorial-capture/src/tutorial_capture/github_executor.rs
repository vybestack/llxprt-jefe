...
///
/// Accepts only `https://github.com/<owner>/<repo>/<issues|pull>/<number>` so
/// every persisted resource remains bound to the same explicit host.
///
...
fn build_commit_command(plan: &FixtureMutationPlan, clone_dest: &Path) -> PlannedCommand {
    PlannedCommand {
        ...
        argv: vec![
            "commit".to_string(),
            "--author".to_string(),
            "jefe-tutorial-capture <tutorial-capture@localhost>".to_string(),
            "-m".to_string(),
...
fn execute_plan_command(
    ...
    if let Err(error) = record_successful_resource(cmd, &stdout, &plan.repository, plan, manifest) {
        return Err(with_mutation_recovery(
            error,
            cmd,
            &stdout,
            plan,
    ...
    if let Err(e) = (ctx.save_fn)(manifest) {
        let error = TierBError::CommandFailed {
            description: "save manifest".to_string(),
            stderr: e,
        };
        return Err(with_mutation_recovery(
...
/// Persist exact identity for reconciling a successful mutation that could
/// not be recorded in the primary manifest.
fn with_mutation_recovery(
    error: TierBError,
    cmd: &PlannedCommand,
    stdout: &str,
    plan: &TierBPlan,
    ...
    let record = serde_json::json!({
        ...
        "issue_title": plan.mutation_plan.issue_title,
        "branch": plan.mutation_plan.branch_name,
        "pr_title": plan.mutation_plan.pr_title,
        "observed_stdout": stdout.trim(),
    });
    let path = run_root.join("github-mutation-recovery.json");
    match write_recovery_record(&path, &record) {
        Ok(()) => error,
        Err(journal_error) => TierBError::CommandFailed {
            description: "persist GitHub mutation recovery record".to_string(),
            stderr: format!("original error: {error}; recovery record failed: {journal_error}"),
        },
    }
}

fn is_remote_mutation(cmd: &PlannedCommand) -> bool {
    cmd.description.starts_with("Create fixture issue")
        || cmd.description.starts_with("Push fixture branch")
        || cmd.description.starts_with("Create fixture PR")
}

fn write_recovery_record(path: &Path, record: &serde_json::Value) -> Result<(), String> {
    use std::io::Write;
    ...
    std::fs::rename(&temp, path).map_err(|err| {
        let _ = std::fs::remove_file(&temp);
        format!("rename recovery record to '{}': {err}", path.display())
    })
}
...
fn validate_parsed_url_matches(
    ...
    if !parsed.repo.eq_ignore_ascii_case(expected_repo) {
        return Err(TierBError::CommandFailed {
            description: cmd.description.clone(),
            stderr: format!(
