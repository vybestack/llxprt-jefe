...
use crate::manifest::{OwnedPath, OwnedPathKind};
...
pub fn validate_owned_paths(owned: &[OwnedPath], run_root: &Path) -> Result<(), PersistenceError> {
    ...
    if let Ok(metadata) = fs::symlink_metadata(run_root)
...
fn check_no_symlink(path: &Path, run_root: &Path) -> Result<(), PersistenceError> {
    ...
    let relative = canonical_path.strip_prefix(&canonical_root).map_err(|_| {
        PersistenceError::PathNotContained {
            path: canonical_path.clone(),
            run_root: canonical_root.clone(),
        }
    })?;
