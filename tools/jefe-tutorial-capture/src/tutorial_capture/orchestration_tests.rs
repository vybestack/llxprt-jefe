...
fn cleanup_skips_nonexistent_paths() {
    ...
    std::fs::remove_dir_all(&dirs.fixture_repo).value_or_panic("pre-remove fixture repo");
...
fn prepare_test_run(base: &Path, profile: RuntimeProfile) -> crate::RunDirectories {
...
fn sample_manifest_for_redaction(dirs: &crate::RunDirectories) -> RunManifest {
    let run_id = RunId::new("redact-test").value_or_panic("valid id");
