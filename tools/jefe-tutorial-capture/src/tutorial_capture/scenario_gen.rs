...
fn build_tier_b_merge_steps(p: &TierBScenarioParams) -> serde_json::Value {
    serde_json::json!([
        ...
        { "key": "m" },
        { "waitFor": format!("Merge Pull Request #{}", p.pr_number) },
        { "expect": p.pr_title },
        { "capture": "merge-confirmation" },

        { "key": "Enter" },
        { "waitFor": "Press Enter to confirm merge" },
        { "expect": format!("Merge Pull Request #{}", p.pr_number) },
        { "expect": p.pr_title },
        { "key": "Enter" },
        { "waitFor": format!("Merged PR #{}", p.pr_number) },
        { "expect": p.pr_title },
...
pub fn validate_tier_b_resources(manifest: &RunManifest) -> Result<(), TierBValidationError> {
    let fixture_repo = manifest.fixture_github_repo.as_deref().ok_or_else(|| {
        TierBValidationError::MissingResource {
            kind: "fixture_github_repo".to_string(),
        }
    })?;
    for kind in [
        ...
        if resource.repository != fixture_repo {
