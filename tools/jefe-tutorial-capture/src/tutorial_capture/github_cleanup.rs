...
pub(super) fn validate_resource_for_cleanup(
    ...
    validate_resource_identifier(resource, manifest)
}
fn validate_resource_identifier(
    ...
    match resource.kind {
        GitHubResourceKind::Branch => {
            ...
            if resource.identifier != expected {
                return Err(format!("branch identifier must equal '{expected}'"));
            }
        }
        GitHubResourceKind::Issue | GitHubResourceKind::PullRequest => {
            let number = resource
                .identifier
                .parse::<u64>()
                .map_err(|_| "issue/PR identifier must be a positive integer".to_string())?;
            if number == 0 {
                return Err("issue/PR identifier must be a positive integer".to_string());
            }
        }
    }
    Ok(())
...
/// Whether a command failure indicates the resource was already absent.
