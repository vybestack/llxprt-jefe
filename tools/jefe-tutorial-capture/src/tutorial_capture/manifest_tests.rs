...
#[test]
fn run_manifest_from_json_rejects_invalid_run_id() {
    let manifest = RunManifest::new(
    ...
    let json = manifest
        .to_json()
        .value_or_panic("serialize valid manifest")
        .replace("\"run-001\"", "\"../unsafe\"");
    let error = error_or_panic(RunManifest::from_json(&json), "invalid run ID must fail");
    assert!(
        error.to_string().contains("invalid run ID"),
...
/// Verify that paths with parent-dir traversal are rejected with a typed
/// error (security invariant maintained).
#[test]
fn artifact_entry_rejects_traversal_with_typed_error() {
    let mut manifest = RunManifest::new(
    ...
    let result = manifest.add_artifact(ArtifactEntry {
        label: "bad".to_string(),
        relative_path: PathBuf::from("../../../etc/passwd"),
        kind: ArtifactKind::ScreenCapture,
    });
    assert!(
        matches!(result, Err(super::ManifestArtifactError::UnsafePath { .. })),
        "traversal paths must be rejected with UnsafePath: {result:?}"
    );
    assert!(
        manifest.artifacts.is_empty(),
        "traversal paths must not be recorded"
    );
}
