//! Tests for CLI subcommand parsing and manifest enrichment.
//!
//! Extracted from `tests.rs` to keep file sizes under the project limit.

use crate::cli_cmd::cli::{CliArgs, Command, ParseError};
use crate::cli_cmd::tmux_helpers;
use jefe_tutorial_capture::{
    GitHubResource, GitHubResourceKind, OwnedPathKind, RunId, RunManifest, RuntimeProfile,
    extract_scenario_params, generate_tier_b_scenario,
};
...
#[test]
fn capture_github_requires_manifest_and_jefe_bin() {
...
fn enrich_manifest_clears_stale_hash_on_scenario_hash_failure() {
    ...
    crate::cli_cmd::tmux_helpers::enrich_manifest_with_capture_metadata(
        &mut manifest,
        &jefe_bin,
        &bad_scenario,
    );
...
fn discover_artifacts_registers_screen_captures_on_failure() {
    ...
    let mut manifest = sample_manifest_for_tmux_request();
    crate::cli_cmd::tmux_helpers::discover_and_register_artifacts(&mut manifest, &artifact_dir);
...
fn discover_artifacts_strict_late_failure_registers_and_marks_failed() {
    use jefe_tutorial_capture::RunOutcome;
...
fn finding3_capture_github_generates_scenario_from_manifest_exact_resources() {
    ...
    manifest.add_github_resource(GitHubResource {
        ...
        title: "[tutorial-capture:run-241] fixture issue for documentation capture".to_string(),
    });
    manifest.add_github_resource(GitHubResource {
    ...
    manifest.add_github_resource(GitHubResource {
        ...
        title: "[tutorial-capture:run-241] fixture pull request".to_string(),
    });
