...
use crate::cli_cmd::cli::{CliArgs, Command, GitHubCaptureMode, ParseError, parse_runtime_profile};
...
fn prepare_accepts_shim_availability() {
    use jefe_tutorial_capture::ShimAvailability;
    let args = parse(&[
        "prepare",
        "--run-id",
        "my-run",
...
fn plan_github_refuses_arbitrary_unconfigured_target() {
    use jefe_tutorial_capture::build_allowlist_from_sources;
...
fn plan_github_invalid_repo_format_detected_by_validator() {
    use jefe_tutorial_capture::is_valid_repo_format;
...
// ── capture-github --merge flag (typed capture mode) ──────────────────

/// capture-github defaults to `GitHubCaptureMode::Standard` (no merge).
#[test]
fn capture_github_defaults_to_standard_mode() {
    let args = parse(&[
        "capture-github",
        "--manifest",
        "manifest.json",
    ...
    match args.command {
        Command::CaptureGithub(opts) => {
            assert_eq!(
                opts.mode,
                GitHubCaptureMode::Standard,
                "default must be standard (no merge)"
            );
        }
...
/// capture-github --merge sets `GitHubCaptureMode::Merge`.
#[test]
fn capture_github_merge_flag_sets_merge_mode() {
    let args = parse(&[
        "capture-github",
        "--manifest",
        "manifest.json",
    ...
    match args.command {
        Command::CaptureGithub(opts) => {
            assert_eq!(
                opts.mode,
                GitHubCaptureMode::Merge,
                "--merge flag must set merge mode"
            );
        }
        other => panic!("expected CaptureGithub, got {other:?}"),
    }
