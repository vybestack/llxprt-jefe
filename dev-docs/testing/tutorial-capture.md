...
Run the opt-in GitHub Issues/PR capture flow using a prepared manifest and
the real Jefe binary:

```bash
cargo run -p jefe-tutorial-capture -- capture-github \
  --manifest /tmp/jefe-tutorial-capture/my-tutorial-run/run-manifest.json \
  --jefe-bin target/debug/jefe
```

The tool generates the executable scenario from the manifest's exact,
current-run issue, branch, and pull-request resources. It writes that scenario
atomically below the run artifact directory and registers it in the manifest;
callers cannot supply a generic GitHub scenario. Add `--merge` only for a run
that was prepared with `plan-github --allow-merge`.
...
local capture scenario does **not** include Issues/PR checkpoints because its
fixture repo has no GitHub configuration.
...
The generated standard scenario confirms issue and PR send-to-agent behavior
against the exact resources recorded by the current run. The generated merge
variant waits for the armed confirmation prompt and rechecks the exact pull
request number and title before sending the final irreversible `Enter`.

### Merge authorization

Merge authorization is persisted in the manifest from `plan-github
--allow-merge` as `merge_authorized: true`. `capture-github --merge` requires
that authorization. Selection is an explicit typed CLI mode; filenames never
authorize a merge. Setup without `--allow-merge`, and capture without
`--merge`, never merge.
...
manifest-bound scenario generated by `capture-github`.
