...
scripts/regenerate-first-agent-tutorial.sh regenerate --root "$RUN_ROOT"
...
publication-safe. The regeneration command validates every selected output
before replacing any committed asset, then copies the generated bytes without
manual SVG editing. `docs/assets/first-agent-tutorial.provenance` records the
source commit/version, a bounded source-contract fingerprint, and each selected
