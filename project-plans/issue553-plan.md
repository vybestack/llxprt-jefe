...
| A8 | Issue send, agent chooser confirm | Send to a local agent | Exactly one authoritative probe per send | Pre-side-effect rejection still precedes prep | Working copy untouched when validation rejects | Unchanged | `issue_send_pre_check_uses_validation_not_preparation` |
