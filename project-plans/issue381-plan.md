...
| 2026-07-24 | S6 writer GREEN checkpoint | 8 focused writer tests pass, including matching/absent hash authority, immutable conflict draft, stale revision cleanup, retained schema-1 backup reuse/conflict/mode, an 11-phase pre-replace fault matrix, and post-rename parent-sync semantics. The persistence worker now supplies revision freshness at the writer replacement boundary, stale candidates are non-authoritative success, and the old PID-temp writer was removed. `make quick-check` passes (2,433 library tests passed, 1 ignored, plus every integration/doc target); writer-local strict Clippy findings are fixed and full library Clippy reaches only the 4 recorded Rust 1.97 baseline findings |
