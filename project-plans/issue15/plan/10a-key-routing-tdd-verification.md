...
- Expected files from previous phase: failing test suite for key routing (26 tests).
...
- THEN all 26 test names exist, tests compile, failures are for unimplemented dispatch (not compilation), tests use state-based assertions, traceability markers present.
...
- [ ] All 26 planned test names exist.
- [ ] Tests compile.
...
- [ ] All REQ-ISS covered in P10 have tests (cross-reference):
  ...
  - REQ-ISS-002: suppression tests (4 keys) + priority tests + input mode tests (3)
  - REQ-ISS-003: navigation tests (down, enter) + pane focus tests (tab, shift+tab)
  - REQ-ISS-004: Esc priority tests (inline > chooser)
  - REQ-ISS-008: filter/search key tests (/, f, f-noop)
  - REQ-ISS-009: open browser test (o)
  - REQ-ISS-010: inline edit/reply tests (e-body, e-comment, r-comment, r-noop)
...
## Success Criteria
- [ ] TDD verification pass (RED tests confirmed, all 26 present).
...
- timestamp
- test name verification output (all 26)
