...
- [ ] Key routing pseudocode covers: 8-level priority chain, suppression rules (all 4 keys), inline key handling, agent chooser keys, search input keys, filter controls keys, issues-global unwind/mode controls, focus-domain handlers, pane-cycle handling, scope change handler (with draft discard), reply `@author` prefill, exclusivity guard, `InputMode` resolution for all 5 issues variants.
