...
fn remote_ssh_args(
    ...
    // Defense in depth: validate SSH identity fields before constructing the
    // destination. The authoritative validation happens at form/persistence
    // boundaries via domain::target::validate_remote, but every SSH command
    // site re-checks so a stale or unvalidated RemoteRepositorySettings can
...
fn local_launch_plan(signature: &LaunchSignature) -> LocalLaunchPlan {
    let mut env = Vec::new();
    let is_llxprt = signature.agent_kind == AgentKind::Llxprt;
    let warning = if is_llxprt && signature.sandbox_enabled {
    ...
    if is_llxprt && !signature.llxprt_debug.is_empty() {
