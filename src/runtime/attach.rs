...
use super::attach_style::{
    base_terminal_style, dim_rgb, fallback_ansi_color, resolve_color, rgb_to_iocraft,
};
...
impl AttachedViewer {
    ...
    fn spawn_command(
        ...
        Self::spawn_pty_child(session_name, pty_pair, cmd, rows, cols)
    }

    /// Spawn the PTY child and assemble the viewer around the pair. Shared
    /// by the production attach path and the test-only idle viewer.
    fn spawn_pty_child(
        session_name: &str,
        pty_pair: PtyPair,
        cmd: CommandBuilder,
    ...
    /// Test-only idle viewer over a bare PTY pair: a `sleep` child instead
    /// of a tmux attach, so manager-level tests can exercise the viewer
    /// resize path without a live session. Unix-only because it shells out
    /// to `sleep`.
    #[cfg(all(test, unix))]
    pub fn idle_for_tests(rows: u16, cols: u16) -> Result<Self, RuntimeError> {
        let pty_pair = open_pty(rows, cols)?;
        let mut cmd = CommandBuilder::new("sleep");
        cmd.arg("30");
        Self::spawn_pty_child("test-idle-viewer", pty_pair, cmd, rows, cols)
    }

    /// Test-only: poison the master lock so every subsequent `resize` fails
    /// deterministically with `ResizeFailed("lock poisoned")`.
    ///
    /// # Panics
    ///
    /// Deliberately panics while holding the master lock so the mutex poisons
    /// during unwind. The panic is caught inside this method; it is the
