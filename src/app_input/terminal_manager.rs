//! Terminal-manager key dispatch and runtime orchestration (issue #361 PR B).
//!
...
use jefe::state::{AppEvent, AppState, ManagedShellRow, ScreenMode, project_managed_shell_rows};
...
/// selected managed shell every ~2 seconds (issue #361 PR B).
///
...
pub async fn complete_pending_shell_focus(
    ...
    let pending = {
        let state = app_state.read();
        if state.screen_mode != ScreenMode::DashboardTerminals {
            return;
        }
        state.terminal_manager.pending_focus.clone()
    };
    ...
    let result = smol::unblock(move || {
        let mut guard = ctx_clone.lock().map_err(|_| {
        ...
        if !guard.runtime.shell_window_exists(&owner)? {
            return Err(jefe::runtime::RuntimeError::SessionNotFound(
                owner.0.clone(),
            ));
        }
...
pub fn on_shell_attach_failed(app_state: &mut AppStateHandle, failed_agent_id: &AgentId) {
    let pending_matches = {
        let state = app_state.read();
        state.screen_mode == ScreenMode::DashboardTerminals
            && state
                .terminal_manager
                .pending_focus
                .as_ref()
