...
fn refresh_issue_navigation(
    ...
    match focus {
        ...
        jefe::state::IssueFocus::IssueList => {
            ...
            load_more_issues_if_at_end(app_state, ctx);
        }
...
fn refresh_repo_scope_if_changed(
    ...
    dispatch_issue_list_fetch(app_state, ctx, true);
}
...
fn load_more_issues_if_at_end(app_state: &mut AppStateHandle, ctx: &SharedContext) {
    let should_load = {
        let state = app_state.read();
        let at_end = state
            .issues_state
...
fn is_fresh_issue_list_reload(message: &IssuesMessage) -> bool {
    matches!(
        message,
