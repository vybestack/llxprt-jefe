...
pub fn keybind_hints_for(screen_mode: ScreenMode, terminal_focused: bool) -> &'static str {
    ...
    match screen_mode {
        ...
        ScreenMode::DashboardIssues => {
            "^/v items | </> panes | Enter detail | n new issue | f filter | / search | Tab detail focus (j/k) | i list | r reply | S send-to-agent | e edit | c comment | C close (reason) | D delete | a exit | Esc back/exit"
        }
