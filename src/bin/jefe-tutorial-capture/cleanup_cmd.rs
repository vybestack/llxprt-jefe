...
use crate::cli::{CleanupOpts, write_stderr, write_stdout};
...
fn handle_cleanup_persistence_failure(
    ...
    records: &[jefe::tutorial_capture::CleanupRecord],
) -> ExitCode {
    ...
    let _ = fs::write(&journal_path, &journal_msg);
    write_stderr(&format!(
...
mod tests {
    use super::*;
    use jefe::tutorial_capture::{RunId, RuntimeProfile};
