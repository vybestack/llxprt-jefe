...
//!
//! The plan-github subcommand is in [`crate::plan_github`].
...
use crate::cli::{
    CaptureGithubOpts, CaptureLocalOpts, PrepareOpts, RenderOpts, ValidateRuntimeOpts,
    parse_runtime_profile, write_stderr, write_stdout,
};

use jefe::tutorial_capture::{
    OwnedPathKind, RunId, RunManifest, RunSetup, load_manifest, prepare_run, save_manifest,
    save_report,
};

use super::tmux_helpers::{render_single_artifact, run_capture_scenario};

pub use crate::plan_github::run_plan_github;
...
fn save_artifact_entry(
    ...
    kind: jefe::tutorial_capture::ArtifactKind,
) -> Result<(), String> {
    let relative_path = relative_path.into();
    let label = label.into();
    jefe::tutorial_capture::validate_artifact_path(&relative_path)
        .map_err(|e| format!("unsafe artifact path '{}': {e}", relative_path.display()))?;
    manifest.add_artifact(jefe::tutorial_capture::ArtifactEntry::new(
        label,
        relative_path,
        kind,
    ));
...
pub fn run_prepare(opts: PrepareOpts) -> ExitCode {
    ...
    if let Err(err) = jefe::tutorial_capture::validate_real_runtime(runtime_profile) {
...
pub fn run_capture_local(opts: CaptureLocalOpts) -> ExitCode {
    ...
    if let Ok(manifest) = load_manifest(&opts.manifest_path)
        && !manifest.runtime_profile.is_shim()
    {
        write_stderr(&format!(
...
fn runtime_profile_name(profile: jefe::tutorial_capture::RuntimeProfile) -> &'static str {
    match profile {
        jefe::tutorial_capture::RuntimeProfile::Shim => "shim",
        jefe::tutorial_capture::RuntimeProfile::RealLlxprt => "real-llxprt",
        jefe::tutorial_capture::RuntimeProfile::RealCodePuppy => "real-code-puppy",
    }
}
...
fn write_generated_scenario(
    ...
    let params = jefe::tutorial_capture::extract_scenario_params(manifest, "TutorialAgent")
        .ok_or_else(|| {
            write_stderr("error: could not extract scenario parameters from manifest resources.\n");
            ExitCode::from(1)
        })?;
    let generated_json = if is_merge {
        jefe::tutorial_capture::generate_tier_b_merge_scenario(&params)
    } else {
        jefe::tutorial_capture::generate_tier_b_scenario(&params)
    };
    ...
    jefe::tutorial_capture::write_artifact_atomic(
        &artifact_dir,
        &scenario_rel,
        &generated_json,
        manifest,
        scenario_label,
        jefe::tutorial_capture::ArtifactKind::Scenario,
    )
    .map_err(|err| {
        write_stderr(&format!(
...
pub fn run_capture_github(opts: CaptureGithubOpts) -> ExitCode {
    ...
    // Finding #2: Fail closed unless the manifest has exact valid resources.
    // The generic static fallback is removed.
    if let Err(err) = jefe::tutorial_capture::validate_tier_b_resources(&manifest) {
        write_stderr(&format!(
    ...
    let is_merge = opts
        ...
        .file_stem()
        .and_then(|s| s.to_str())
        .is_some_and(|stem| stem.contains("merge"));
    ...
    crate::tmux_helpers::run_capture_github_scenario(
        &opts.manifest_path,
        &generated_path,
        &opts.jefe_bin,
        opts.keep_session,
...
/// fixture GitHub repo is set, merge authorization is present for merge scenarios.
...
fn validate_capture_github_prerequisites(
    ...
    let fixture_clone = manifest_dir.join("fixture-clone");
    if !fixture_clone.exists() {
        write_stderr(&format!(
            "error: fixture clone not found at '{}'. Run 'plan-github --confirm-disposable' first to clone the fixture repo.\n",
            fixture_clone.display()
        ));
    ...
    let is_merge_scenario = opts
        .scenario_path
        .file_stem()
        .and_then(|s| s.to_str())
        .is_some_and(|stem| stem.contains("merge"));
    if is_merge_scenario && !manifest.merge_authorized {
        write_stderr(
            "error: merge scenario requires merge authorization. \
             Run 'plan-github --allow-merge' to authorize merge.\n",
        );
        return Err(ExitCode::from(1));
    }
    let agent_kind = jefe::tutorial_capture::derive_agent_kind(
        manifest.runtime_profile,
        manifest.shim_availability,
    );
    let seed = jefe::tutorial_capture::TierBStateSeed {
        config_dir,
    ...
    if let Err(err) = jefe::tutorial_capture::seed_tier_b_state(&seed) {
...
pub fn run_validate_runtime(opts: ValidateRuntimeOpts) -> ExitCode {
    ...
    if let Err(err) = jefe::tutorial_capture::validate_real_runtime(manifest.runtime_profile) {
        write_stderr(&format!("runtime profile error: {err}\n"));
        return ExitCode::from(1);
    }
    let runtime_name =
        jefe::tutorial_capture::runtime_binary_name(manifest.runtime_profile).unwrap_or("unknown");
    write_stdout(&format!(
    ...
    let artifact_dir = manifest_dir.join("artifacts");
    let scenario_path = match jefe::tutorial_capture::prepare_validate_runtime_scenario(
        &mut manifest,
        &artifact_dir,
        run_root,
    ) {
    ...
    super::tmux_helpers::run_validate_runtime_scenario(
        &opts.manifest_path,
        &scenario_path,
        &opts.jefe_bin,
        opts.keep_session,
            ...
            Ok(svgs) => {
                if let Some(mono_svg) = svgs.mono_svg {
                    ...
                    if let Err(err) = save_artifact_entry(
                        ...
                        mono_svg,
                        jefe::tutorial_capture::ArtifactKind::Visual,
                    ) {
                ...
                if let Some(color_svg) = svgs.color_svg
                    && let Err(err) = save_artifact_entry(
                        ...
                        color_svg,
                        jefe::tutorial_capture::ArtifactKind::ColorSvg,
                    )
                {
                    write_stderr(&format!(
...
pub fn run_report(manifest_path: &Path) -> ExitCode {
    ...
    if let Err(err) = save_artifact_entry(
        ...
        "run-report",
        &report_relative,
        jefe::tutorial_capture::ArtifactKind::Report,
    ) {
        write_stderr(&format!(
