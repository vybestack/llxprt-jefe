...
pub struct PrepareOpts {
    ...
    pub shim_availability: jefe::tutorial_capture::ShimAvailability,
}
    ...
    I: Iterator<Item = String>,
{
    ...
    let shim_availability = match &shim_availability {
        Some(value) => jefe::tutorial_capture::ShimAvailability::parse(value).ok_or_else(|| {
            ParseError::message(format!(
                "unknown shim availability '{value}'. Valid: llxprt-only, code-puppy-only, both"
            ))
        })?,
        None => jefe::tutorial_capture::ShimAvailability::default(),
    };
    Ok(PrepareOpts {
