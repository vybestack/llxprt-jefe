...
use crate::cli::{PlanGithubOpts, write_stderr, write_stdout};
...
fn build_plan_allowlist(
    opts: &PlanGithubOpts,
) -> Result<jefe::tutorial_capture::FixtureAllowlist, ExitCode> {
...
fn resolve_clone_dest(opts: &PlanGithubOpts) -> PathBuf {
    opts.clone_dest.clone().unwrap_or_else(|| {
        if let Some(manifest_path) = &opts.manifest_path {
            ...
            PathBuf::from(format!("/tmp/jefe-tutorial-{}/clone", opts.run_id))
        }
    })
}
...
pub fn run_plan_github(opts: PlanGithubOpts) -> ExitCode {
    ...
    if !jefe::tutorial_capture::is_valid_repo_format(&opts.fixture_repo) {
        write_stderr(&format!(
...
fn execute_plan_github(
    ...
    plan: &TierBPlan,
    clone_dest: &Path,
    allowlist: &jefe::tutorial_capture::FixtureAllowlist,
    run_id: &RunId,
) -> ExitCode {
    ...
    record_tier_b_paths(&mut manifest, clone_dest, plan);
...
fn finalize_tier_b_execution(
    result: Result<Vec<String>, jefe::tutorial_capture::TierBError>,
...
fn pre_execution_checks(clone_dest: &Path, run_root: &Path) -> Result<(), ExitCode> {
    ...
    let tier_b_missing = jefe::tutorial_capture::check_tier_b_required_tools(
    ...
    if let Err(err) = jefe::tutorial_capture::validate_clone_destination(clone_dest, run_root) {
        write_stderr(&format!(
...
fn record_tier_b_paths(manifest: &mut RunManifest, clone_dest: &Path, plan: &TierBPlan) {
    ...
    // Associate the GitHub fixture repo from the plan.
    manifest.set_fixture_github_repo(&plan.repository);
}
