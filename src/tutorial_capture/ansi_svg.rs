...
fn parse_ansi_segments(line: &str, palette256: &[String]) -> Vec<AnsiSegment> {
    ...
    while i < bytes.len() {
        ...
        if i + 1 < bytes.len() && bytes[i] == 0x1b && bytes[i + 1] == b'[' {
            ...
            apply_sgr_params(&param_str, &mut current, palette256);
        } else {
            // Regular character.
            let ch = bytes[i];
            if ch < 0x80 {
...
fn write_color_metadata_comment(svg: &mut String, metadata: &ColorSvgMetadata) {
    let _ = writeln!(
        svg,
        "<!-- color-preserving-svg: cell={}x{} padding={} font_size={} font_family=\"{}\" declared_rows={} declared_cols={} theme={} jefe={}{} -->",
        CELL_WIDTH,
        CELL_HEIGHT,
        PADDING,
        FONT_SIZE,
