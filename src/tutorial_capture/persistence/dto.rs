...
use crate::tutorial_capture::path_shim::ShimAvailability;
...
pub fn dto_to_manifest(
    ...
    if let Some(p) = &dto.fixture_repo_path {
        manifest.set_fixture_repo(p.clone());
    }
    ...
    for artifact in &dto.artifacts {
        manifest.add_artifact(artifact.clone());
    }
