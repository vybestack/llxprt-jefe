...
fn shim_availability_both_round_trips_through_dto() {
    ...
    manifest.shim_availability = crate::tutorial_capture::path_shim::ShimAvailability::Both;
    save_manifest_atomic(&manifest, &run_root).value_or_panic("save");

    let loaded = load_and_validate(&run_root).value_or_panic("load");
    assert_eq!(
        loaded.shim_availability,
        crate::tutorial_capture::path_shim::ShimAvailability::Both,
...
fn shim_availability_llxprt_only_round_trips_through_dto() {
    ...
    let mut manifest = make_valid_manifest(&run_root);
    manifest.shim_availability = crate::tutorial_capture::path_shim::ShimAvailability::LlxprtOnly;
    save_manifest_atomic(&manifest, &run_root).value_or_panic("save");

    let loaded = load_and_validate(&run_root).value_or_panic("load");
    assert_eq!(
        loaded.shim_availability,
        crate::tutorial_capture::path_shim::ShimAvailability::LlxprtOnly,
...
fn shim_availability_code_puppy_only_round_trips_through_dto() {
    ...
    manifest.shim_availability =
        crate::tutorial_capture::path_shim::ShimAvailability::CodePuppyOnly;
    save_manifest_atomic(&manifest, &run_root).value_or_panic("save");
