...
use super::*;
use crate::tutorial_capture::manifest::RuntimeProfile;
...
fn scenario_parses_through_harness_parse_scenario() {
    for profile in [RuntimeProfile::RealLlxprt, RuntimeProfile::RealCodePuppy] {
        let json = generate_validate_runtime_scenario(profile)
            .value_or_panic(&format!("scenario for {profile:?} should generate"));
        crate::harness::parse_scenario(&json).value_or_panic(&format!(
...
fn setup_test_run(
    ...
    std::path::PathBuf,
    std::path::PathBuf,
    crate::tutorial_capture::manifest::RunManifest,
) {
    ...
    let run_id =
        crate::tutorial_capture::manifest::RunId::new(run_label).value_or_panic("valid run id");
    let mut manifest = crate::tutorial_capture::manifest::RunManifest::new(
        run_id,
        "0.0.28",
        "validate-runtime",
        100,
    ...
    manifest.add_owned_path(
        crate::tutorial_capture::manifest::OwnedPathKind::ArtifactDir,
        artifact_dir.clone(),
    );
    manifest.add_owned_path(
        crate::tutorial_capture::manifest::OwnedPathKind::ConfigDir,
...
fn prepare_scenario_writes_under_artifacts_scenarios_and_registers() {
    ...
    let scenario_artifact = manifest
        .artifacts
        .iter()
        .find(|a| a.kind == crate::tutorial_capture::manifest::ArtifactKind::Scenario)
    ...
    assert!(
        loaded
            .artifacts
            .iter()
            .any(|a| a.kind == crate::tutorial_capture::manifest::ArtifactKind::Scenario),
