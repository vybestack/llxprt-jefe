//! Terminal-manager reducer operations (issue #364 PR A).
//!
...
pub fn project_managed_shell_rows(state: &AppState) -> Vec<ManagedShellRow> {
    state
        ...
        .map(|agent_id| {
            ...
            let repository = agent.and_then(|owner| state.repository_by_id(&owner.repository_id));
            ManagedShellRow {
                agent_id: agent_id.clone(),
                agent_name: agent.map_or_else(
                    || format!("Unknown owner ({})", agent_id.0),
                    |owner| owner.name.clone(),
                ),
                repository_name: repository.map_or_else(
                    || "Unknown repository".to_string(),
                    |repo| repo.name.clone(),
                ),
                repository_id: repository.map_or_else(
                    || crate::domain::RepositoryId(String::new()),
                    |repo| repo.id.clone(),
                ),
                work_dir: agent.map_or_else(
                    || "Unknown worktree".to_string(),
                    |owner| owner.work_dir.to_string_lossy().into_owned(),
                ),
                status_label: agent.map_or_else(
                    || "Unknown".to_string(),
...
impl AppState {
    ...
    /// Record a generation-guarded pending focus request (issue #364 PR A).
    ...
    fn request_shell_focus(&mut self, agent_id: AgentId, origin: ShellFocusOrigin) -> bool {
        let resumable = match origin {
            ...
            ShellFocusOrigin::DashboardF10 => self.agents.iter().any(|agent| {
                agent.id == agent_id
                    ...
                    && self.has_shell_window(&agent_id)
            }),
        };
        if !resumable {
            return true;
    ...
    /// Confirm a pending focus after the expected owner attached (issue #364
    /// PR A). Generation-guarded: a mismatched owner or stale generation is
    ...
    /// Handle all Terminal Manager events (issue #364 PR A).
    pub(super) fn apply_terminal_manager_message(
        ...
        match message {
            ...
            TerminalManagerMessage::RequestFocus { agent_id, origin } => {
                self.request_shell_focus(agent_id, origin)
            }
...
mod tests {
    ...
    fn request_focus_records_generation_guarded_pending() {
        ...
        state.apply_terminal_manager_message(TerminalManagerMessage::RequestFocus {
    ...
    #[test]
    fn boundary_navigation_invalidates_pending_focus() {
        let mut state = state_with_two_shells();
        state.apply_terminal_manager_message(TerminalManagerMessage::EnterMode);
        state.apply_terminal_manager_message(TerminalManagerMessage::RequestFocus {
            agent_id: AgentId("agent-1".into()),
        ...
        state.apply_terminal_manager_message(TerminalManagerMessage::Navigate(NavDir::Up));

        assert!(state.terminal_manager.pending_focus.is_none());
        assert_eq!(
            state.terminal_manager.generation,
