...
fn assert_blocked_inline_open_preserves_state(event: AppEvent) {
    let mut state = issues_mode_state_with_repo("repo-1");
    state.issues_state.draft_notice = Some("No agents available".to_string());
...
fn blocked_inline_opens_preserve_draft_notice_and_active_editor() {
    assert_blocked_inline_open_preserves_state(AppEvent::OpenNewIssueComposer);
    assert_blocked_inline_open_preserves_state(AppEvent::OpenNewCommentComposer);
    assert_blocked_inline_open_preserves_state(AppEvent::OpenReplyComposer { comment_index: 0 });
    assert_blocked_inline_open_preserves_state(AppEvent::OpenInlineEditor {
        target: EditorTarget::IssueBody,
    });
}
