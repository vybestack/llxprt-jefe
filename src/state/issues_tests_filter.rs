...
// ── issue #241 Finding #1: Tier B issue search flow behavioral test ──
//
// The Tier B scenario presses `/` to focus the issue search input, then types
// the exact fixture issue title character by character (each keystroke emits
// SetSearchQuery), then presses Enter to apply the search filter. This test
// verifies the reducer correctly accumulates the query and commits it to
// committed_filter.query_text on ApplySearch.

/// Simulate pressing `/` to focus search, then typing the exact fixture issue
/// title one character at a time, then pressing Enter to apply. The committed
/// filter must match the exact title and trigger a list reload.
///
/// @requirement REQ-ISS-007 (issue #241 Finding #1)
#[test]
fn test_tier_b_issue_search_flow_types_exact_title_and_applies_filter() {
    use crate::messages::IssuesMessage;

    // Start in dashboard issues mode with issues active.
    let mut state = dashboard_issues_state();
    state.issues_state.active = true;

    // Step 1: Press `/` to focus the search input.
    let state = state.apply(AppEvent::FocusSearchInput);
    assert!(
        state.issues_state.search_input_focused,
        "search input must be focused after FocusSearchInput"
    );

    // Step 2: Type the exact fixture issue title character by character.
    let exact_title = "[tutorial-capture:run-001] fixture issue";
    let mut state = state;
    for ch in exact_title.chars() {
        let mut query = state.issues_state.search_query.clone();
        query.push(ch);
        state = state.apply(AppEvent::SetSearchQuery { query });
    }

    // The search_query must accumulate the exact title.
    assert_eq!(
        state.issues_state.search_query, exact_title,
        "search_query must accumulate the exact issue title character by character"
    );

    // Step 3: Press Enter to apply the search (IssuesMessage::ApplySearch).
    let state = state.apply(AppEvent::from(IssuesMessage::ApplySearch));

    // The committed filter must contain the exact title.
    assert_eq!(
        state.issues_state.committed_filter.query_text, exact_title,
        "committed_filter.query_text must match the exact typed title after ApplySearch"
    );

    // The search input must be blurred after applying.
    assert!(
        !state.issues_state.search_input_focused,
        "search input must be blurred after ApplySearch"
    );

    // A list reload must be triggered.
    assert!(
        state.issues_state.loading.list,
        "ApplySearch must trigger a list reload"
    );

    // The issues list must be cleared (stale results removed).
    assert!(
        state.issues_state.issues.is_empty(),
        "ApplySearch must clear the stale issues list"
    );
}

/// Typing a partial title into the search box and applying must commit the
/// partial query (the filter narrows but may not match exactly). This proves
/// the filter is driven by the typed text, not a hardcoded value.
///
/// @requirement REQ-ISS-007 (issue #241 Finding #1)
#[test]
fn test_tier_b_issue_search_partial_title_commits_partial_query() {
    use crate::messages::IssuesMessage;
    ...
    // Type only part of the title.
    let partial = "[tutorial-capture";
    let mut state = state;
    ...
    let state = state.apply(AppEvent::from(IssuesMessage::ApplySearch));
    assert_eq!(
        state.issues_state.committed_filter.query_text, partial,
        "committed_filter must match the partial typed query"
    );
}

/// Backspace during search removes the last character from the query,
/// matching the reducer's SetSearchQuery pop behavior.
///
/// @requirement REQ-ISS-007 (issue #241 Finding #1)
#[test]
fn test_tier_b_issue_search_backspace_removes_last_char() {
    let mut state = dashboard_issues_state();
    state.issues_state.active = true;
    let state = state.apply(AppEvent::FocusSearchInput);
    ...
    assert_eq!(state.issues_state.search_query, "hello");

    // Backspace removes the last char.
    let mut query = state.issues_state.search_query.clone();
    query.pop();
    let state = state.apply(AppEvent::SetSearchQuery { query });
    assert_eq!(state.issues_state.search_query, "hell");
}
