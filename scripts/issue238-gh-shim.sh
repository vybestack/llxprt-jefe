...
JSON
    ...
    # Review threads query (has "reviewThreads") — MUST be checked before the
    # PR comments matcher because the production review-threads query also
    # contains "pullRequest(number:", so a pullRequest-first check would
    # shadow it and return the wrong (empty comments) JSON.
...
JSON
    ...
    # PR comments query (contains "comments(first:" on pullRequest, no reviewThreads)
...
# Match by scanning the full argv for "pr view" + "238" + "--json" rather
# than relying on positional $3, which would break if get_pull_request_detail
# ever reorders its argument vector.
if [[ "$1 $2" == "pr view" ]]; then
    has_number=0
    has_json=0
    for arg in "${@:3}"; do
