...
fn build_tier_b_setup(run_id: &RunId, base: &std::path::Path) -> jefe::tutorial_capture::RunSetup {
    jefe::tutorial_capture::RunSetup {
        run_id: run_id.clone(),
        ...
        shim_availability: jefe::tutorial_capture::ShimAvailability::default(),
    }
}

/// Execute fake Tier B and return the run root + manifest.
fn execute_fake_tier_b(
    dirs: &jefe::tutorial_capture::RunDirectories,
...
fn fake_tier_b_full_path_with_versioned_persistence_reload_and_cleanup() {
    ...
    let reloaded = jefe::tutorial_capture::load_and_validate(&run_root)
        .value_or_panic("reload should succeed");
    assert_eq!(
    ...
    let records =
        jefe::tutorial_capture::cleanup_manifest_with_root(&mut cleanup_manifest, &run_root, false)
