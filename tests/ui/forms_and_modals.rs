...
fn open_confirm_delete_agent_modal() {
    ...
    state = state
        .apply(AppEvent::OpenDeleteAgent(agent_id.clone()))
        .committed_pure();
...
fn open_edit_repository_modal() {
    ...
    state = state
        .apply(AppEvent::OpenEditRepository(repo_id.clone()))
        .committed_pure();
...
fn repository_form_cursor_moves_and_inserts_in_place() {
    ...
    state = state.apply(AppEvent::FormChar('c')).committed_pure();
    state = state.apply(AppEvent::FormMoveCursorLeft).committed_pure();
...
fn repository_form_toggles_remote_fields() {
    ...
    state = state.apply(AppEvent::OpenNewRepository).committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Base Dir
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Default Profile
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Default Agent Kind (hidden Code Puppy model skipped)
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Default LLxprt Mode
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Default LLxprt Version
    state = state.apply(AppEvent::FormNextField).committed_pure(); // GitHub Repo
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Issues / PRs Repo
    state = state.apply(AppEvent::FormNextField).committed_pure(); // RemoteEnabled
    state = state.apply(AppEvent::FormToggleCheckbox).committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // LoginUser
    state = state.apply(AppEvent::FormChar('o')).committed_pure();
    state = state.apply(AppEvent::FormChar('p')).committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Host
    state = state.apply(AppEvent::FormChar('1')).committed_pure();
    state = state.apply(AppEvent::FormChar('0')).committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // SshPort
    state = state.apply(AppEvent::FormNextField).committed_pure(); // IdentityFile
    state = state.apply(AppEvent::FormNextField).committed_pure(); // SshOptions
    state = state.apply(AppEvent::FormNextField).committed_pure(); // RunAsUser
    state = state.apply(AppEvent::FormChar('m')).committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // SetupEnvDefault
    state = state.apply(AppEvent::FormToggleCheckbox).committed_pure();
...
fn agent_form_cursor_delete_and_backspace_are_caret_based() {
    ...
    state = state
        ...
        .committed_pure();
    state = state.apply(AppEvent::FormNextField).committed_pure(); // Name
    state = state.apply(AppEvent::FormChar('a')).committed_pure();
    state = state.apply(AppEvent::FormChar('b')).committed_pure();
    state = state.apply(AppEvent::FormChar('c')).committed_pure();
    state = state.apply(AppEvent::FormMoveCursorLeft).committed_pure(); // ab|c
    state = state.apply(AppEvent::FormDelete).committed_pure(); // remove c => ab|
    state = state.apply(AppEvent::FormBackspace).committed_pure(); // remove b => a|
